...
describe('Tree Math Spec', () => {
    ...
    it('has an array of values representing x and y', () => {
        const expectedX = 42;
        const expectedY = 58;
        testVec2 = new tree_math_1.Vector2(expectedX, expectedY);
    ...
    it('adds two vectors together', () => {
        ...
        const expectedBX = 3;
        const expectedBY = 9;
        let aVec = new tree_math_1.Vector2(expectedAX, expectedAY);
        let bVec = new tree_math_1.Vector2(expectedBX, expectedBY);
        const actualResult = aVec.add(bVec);
        const expectedResult = new tree_math_1.Vector2(expectedAX + expectedBX, expectedAY + expectedBY);
    ...
    it('subtracts one vector from another, proving not commutative', () => {
        ...
        const expectedResult = new tree_math_1.Vector2(expectedBX - expectedAX, expectedBY - expectedAY);
        expect(actualResult.values).toEqual(expectedResult.values);
    });
    it('returns the magnitude of a vector', () => {
        testVec2 = new tree_math_1.Vector2(3, 4);
    ...
    describe('normalizing a vector', () => {
        it('normalizes a unit vector', () => {
            testVec2 = new tree_math_1.Vector2(1, 1);
            const expectedNormalize = new tree_math_1.Vector2(0.7071067811865475, 0.7071067811865475);
        ...
        it('normalizes a non unit vector', () => {
            testVec2 = new tree_math_1.Vector2(1523, 600);
            const mag = testVec2.magnitude();
            const expectedNormalize = new tree_math_1.Vector2(testVec2.values[0] / mag, testVec2.values[1] / mag);
            const actualNormalized = testVec2.normalize();
            expect(actualNormalized).toEqual(expectedNormalize);
        });
        ...
        it('wont attempt to normalized a vector with a magnitude of 0 or less', () => {
            testVec2 = new tree_math_1.Vector2(0, 0);
        ...
        it('gives the clockwise normal to a vector', () => {
            const aVec = new tree_math_1.Vector2(1, 0);
            const clockWiseNormal = aVec.normal(tree_math_1.ClockDirection.CW);
            expect(clockWiseNormal).toEqual(new tree_math_1.Vector2(0, -1));
        });
        it('gives the counterclockwise normal to a vector', () => {
            const aVec = new tree_math_1.Vector2(1, 0);
            const counterClockwiseNormal = aVec.normal(tree_math_1.ClockDirection.CCW);
            expect(counterClockwiseNormal).toEqual(new tree_math_1.Vector2(-0, 1));
        });
    });
    describe('dot product', () => {
        it('returns negative if 2 vectors are pointing away', () => {
            const aVec = new tree_math_1.Vector2(2, 21);
            const bVec = new tree_math_1.Vector2(-23, -25);
            const actualDot = aVec.dot(bVec);
            expect(actualDot).toBeLessThan(0);
        });
        it('returns 0 if 2 vectors are perpendicular', () => {
            const aVec = new tree_math_1.Vector2(17, 17);
            const bVec = new tree_math_1.Vector2(-17, 17);
            const actualDot = aVec.dot(bVec);
            expect(actualDot).toEqual(0);
        });
        it('returns positive if 2 vectors are pointing the same direction', () => {
            const aVec = new tree_math_1.Vector2(16, 12);
            const bVec = new tree_math_1.Vector2(42, 13);
            const actualDot = aVec.dot(bVec);
            expect(actualDot).toBeGreaterThan(0);
        });
        it('finds the angle between two vectors', () => {
            const aVec = new tree_math_1.Vector2(1, 1);
            const bVec = new tree_math_1.Vector2(-1, 1);
